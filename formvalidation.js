...
function validPassword(){
    ...
    if(password==""){
        ...
        displayMsg('password must be more than 7 characters','passwordMsg','red')
        return false
    }
    else if(!password.match(/^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[@#?!%$&*]).{8,50}$/)){
        displayMsg('must contain one uppercase, one lowercase, one numeric, one special character and must be more than one characters','passwordMsg','red')
        return truefalse
    }
    else{
